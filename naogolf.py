...
def TurnAfterHitball1():
    motionPrx.angleInterpolationWithSpeed("HeadYaw", 0.0, 0.5) #movimento collo
...
def firstSearchredball():
    ...
    _searchBallTimes += 1 #viene incrementato il tempo di ricerca ogni secondo
    ...
    while search <= -searchRange:
        findballinfo = findball(search) #ricerca palla in base alla posizione del collo
        if findballinfo != []:
            return findballinfo
        else:
            search += 60
    ...
    motionPrx.moveTo(0.25, 0.0, 0.0, _stepStatue)  #In cerca di correzione della distanza
...
def findball(angle, sub_angle=0):  #Restituisce i dati della palla rossa
    ...
    motionPrx.angleInterpolationWithSpeed("HeadYaw", angle * math.pi / 180, 0.8)  #math.pi Pi / 180 è la formula di conversione radiante
    ...
    if (ballData != []):
        ...
        allballData = [headangle, ballData, 0] #ritorna l'angolo del collo,i dati dell palla e 0 per far finire l'iterazione della ricerca
        return allballData
    else:
        print("Non trovato")
        return []
...
TurnAfterHitball1()
while True:
    allballData = firstSearchredball()  #Posizionamento palla rossa Restituisce informazioni sulla posizione della palla rossa
    if (allballData[2] == 0):
        break
