...
def firstricercaredball():
    ...
    while ricerca <= -ricercaRange:
        findballinfo = findball(ricerca)
        if findballinfo != []:
            return findballinfo
        else:
            ricerca += 60
...
def CalculateRobotToRedball(allballData):
    ...
    if val1 != []:
        ...
        if abs(val1[1][1][0]) >= 0.08:
            motionPrx.moveTo(0, 0, val1[1][1][0], _passoRobot)
    ...
    if x >= 0:
        motionPrx.moveTo(x, 0, 0, _passoRobot)
    ...
    if val3 == []:
        ...
        val3 = findball(0, 30)
    if val3 == []:
        motionPrx.moveTo(0.15, 0, 0, _passoRobot2)
        val3 = findball(0, 30)
    ...
    if val3 != []:
        ...
        motionPrx.moveTo(x, 0, theta, _passoRobot2)
        tts.say("我应该到10厘米了")
        ...
        if val4 != []:
            if abs(val4[1][0][1]) >= 0.08:
                motionPrx.moveTo(0, 0, val4[1][1][1], _passoRobot2)
            elif val4[1][0][1] > 0:
        ...
        if val5 != []:
            ...
            motionPrx.moveTo(0, 0, dy, _passoRobot2)
        ...
        while True:
            allballData = firstricercaredball()
            if (allballData[2] == 0):
                break
...
def secondricercaNAOmark(robotTolandmarkdata):
    camProxy.setActiveCamera(0)
    motionPrx.angleInterpolationWithSpeed("HeadPitch", -0.1, 0.3)
    landmarkProxy.subscribe("landmarkTest")
...
def TurnAfterHitball1():
    ...
    global _passoRobot
    motionPrx.setMoveArmsEnabled(False, False)
    motionPrx.moveTo(0.0, 0.0, -1.33, _passoRobot)
    motionPrx.setMoveArmsEnabled(False, False)
    for i in range(4):
...
def TurnAfterHitball2():
    ...
    motionPrx.moveTo(0.15, 0.0, 0.0, _passoRobot)
    time.sleep(0.5)

    motionPrx.moveTo(0.0, 0.0, -0.5 * math.pi, _passoRobot)
    time.sleep(0.5)

    # 直走0.6找次黄杆
    motionPrx.moveTo(0.3, 0.0, 0.0, _passoRobot)
    time.sleep(0.5)
    motionPrx.moveTo(0.3, 0.0, 0.0, _passoRobot)
...
#zhuagan()  # mazza da golf

#headtouch()  # Tocca la fronte per iniziare
#time.sleep(1)
...
while True:
    allballData = firstricercaredball()  # 红球定位 返回红球位置信息
    if (allballData[2] == 0):
        break
...
while True:
    ...
    allmarkdata = firstricercaNAOmark()  # NAOmark定位
    if (allmarkdata[4] == 0):
        ...
        h = 0.478
        allmarkdata = secondricercaNAOmark(robotTolandmarkdata)
        if (allmarkdata[4] == 0):
